...
export default function Submission() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus({
        type: 'success',
        message: 'Submissão realizada com sucesso! Você receberá um e-mail de confirmação em breve.',
      });
      ...
      <div className="container max-w-4xl mx-auto px-6">
        ...
        <form onSubmit={handleSubmit} className="bg-[#131a2a] border border-[rgba(79,142,247,0.14)] rounded-lg p-8">
          ...
          <button
            ...
            className="w-full px-6 py-4 bg-gradient-to-r from-[#4f8ef7] to-[#7c55e8] text-white font-bold rounded-lg hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {status.type === 'loading' ? 'Enviando...' : 'Enviar Submissão'}
          </button>
        </form>
